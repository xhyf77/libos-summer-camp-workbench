...
uint8_t vgic_int_ptarget_mask(struct vcpu *vcpu, struct vgic_int *interrupt) {
    if (vgic_broadcast(vcpu, interrupt)) {
        return cpu()->vcpu->vm->cpus & ~(1U << cpu()->vcpu->p_id);
    } else {
...
bool vgic_int_set_route(struct vcpu *vcpu, struct vgic_int *interrupt, 
    ...
    if (route & GICD_IROUTER_IRM_BIT) {
        phys_route = cpu_id_to_mpidr(vcpu->p_id);
    } else {
        tvcpu =
            vm_get_vcpu_by_mpidr(vcpu->vm, route & MPIDR_AFF_MSK);
        if (tvcpu != NULL) {
            phys_route = cpu_id_to_mpidr(tvcpu->p_id) & MPIDR_AFF_MSK;
        } else {
...
void vgicr_emul_typer_access(struct emul_access *acc,
    ...
    if (!acc->write) {
        vcpu = vm_get_vcpu(cpu()->vcpu->vm, vgicr_id);
        typer = vcpu->arch.vgic_priv.vgicr.TYPER;
...
void vgicr_emul_pidr_access(struct emul_access *acc,
    ...
    if (!acc->write) {
        val = 0;
        pgicr_id = vm_translate_to_pcpuid(cpu()->vcpu->vm, vgicr_id);
...
void vgicd_emul_router_access(struct emul_access *acc,
    ...
    if (!acc->write) {
        ...
        reg_value = vcpu_readreg(cpu()->vcpu, acc->reg);
        if (top_access) {
...
bool vgicr_emul_handler(struct emul_access *acc) {
    ...
    switch (GICR_REG_MASK(acc->addr)) {
        ...
        case GICR_REG_OFF(ICFGR1):
            ...
            break;
        default: {
            base_offset = acc->addr - cpu()->vcpu->vm->arch.vgicr_addr;
            acc_offset = GICR_REG_MASK(base_offset);
    ...
    if (vgic_check_reg_alignment(acc, handler_info)) {
        vgicr_id = vgicr_get_id(acc);
        vcpu = vgicr_id == cpu()->vcpu->id
                           ? cpu()->vcpu
...
bool vgic_icc_sgir_handler(struct emul_access *acc) {
    ...
    if (acc->write) {
        sgir = vcpu_readreg(cpu()->vcpu, acc->reg);
        if (acc->multi_reg) {
            sgir_high = vcpu_readreg(cpu()->vcpu, acc->reg_high); 
            sgir |= (sgir_high<<32);
        }
        int_id = ICC_SGIR_SGIINTID(sgir);
        if (sgir & ICC_SGIR_IRM_BIT) {
            trgtlist = cpu()->vcpu->vm->cpus & ~(1U << cpu()->vcpu->p_id);
        } else {
            /**
            ...
            trgtlist = vm_translate_to_pcpu_mask(
                cpu()->vcpu->vm, ICC_SGIR_TRGLSTFLT(sgir), cpu()->vcpu->vm->nr_cpus);
        }
...
void vgic_init(struct vm *vm, const struct vgic_dscrp *vgic_dscrp) {
    ...
    vm->arch.vgicd.TYPER =
        ...
        (((vm->nr_cpus - 1) << GICD_TYPER_CPUNUM_OFF) & GICD_TYPER_CPUNUM_MSK) |
        (((10 - 1) << GICD_TYPER_IDBITS_OFF) & GICD_TYPER_IDBITS_MSK);
    vm->arch.vgicd.IIDR = gicd->IIDR;

    vgic_int_size = vm->arch.vgicd.int_num * sizeof(struct vgic_int);
    vm->arch.vgicd.interrupts =
        mem_alloc_page(NUM_PAGES(vgic_int_size), false);
    ...
    INIT_LIST_HEAD(&vm->arch.vgicd_emul.list);
    vm_emul_add_mem(vm, &vm->arch.vgicd_emul);

    for (vcpuid_t vcpuid = 0; vcpuid < vm->nr_cpus; vcpuid++)
    {
        vcpu = vm_get_vcpu(vm, vcpuid);
        typer = (uint64_t)vcpu->id << GICR_TYPER_PRCNUM_OFF;
        typer |= ((uint64_t)vcpu->arch.vmpidr & MPIDR_AFF_MSK) << GICR_TYPER_AFFVAL_OFF;
        typer |= !!(vcpu->id == vcpu->vm->nr_cpus - 1) << GICR_TYPER_LAST_OFF;
    ...
    vm->arch.vgicr_emul = (struct emul_mem) {
        ...
        .size = ALIGN(sizeof(struct gicr_hw), PAGE_SIZE) * vm->nr_cpus,
        .handler = vgicr_emul_handler
    };
    INIT_LIST_HEAD(&vm->arch.vgicr_emul.list);
    ...
    INIT_LIST_HEAD(&vm->arch.icc_sgir_emul.list);
    ...
    INIT_LIST_HEAD(&vm->arch.icc_sre_emul.list);
    vm_emul_add_reg(vm, &vm->arch.icc_sre_emul);

    INIT_LIST_HEAD(&vm->arch.vgic_spilled);
...
void vgic_cpu_init(struct vcpu *vcpu) {
    for (size_t i = 0; i < GIC_CPU_PRIV; i++) {
        ...
        vcpu->arch.vgic_priv.interrupts[i].phys.redist = vcpu->p_id;                              
    ...
    INIT_LIST_HEAD(&vcpu->arch.vgic_spilled);
}
