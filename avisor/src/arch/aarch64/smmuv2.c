...
void smmu_init() {
     ...
     */
    vaddr_t smmu_glbl_rs0 = platform.arch.smmu.base;
    ...
    vaddr_t smmu_glbl_rs1 = platform.arch.smmu.base + pg_size;

    vaddr_t smmu_cntxt = platform.arch.smmu.base + (num_page * pg_size);
...
size_t smmu_alloc_ctxbnk() {
    spin_lock(&smmu.ctx_lock);
    /* Find a free context bank. */
    size_t nth = bitmap_find_nth(smmu.ctxbank_bitmap, smmu.ctx_num, 1, 0, false);
    if (nth >= 0) {
...
size_t smmu_alloc_sme() {
    spin_lock(&smmu.sme_lock);
    /* Find a free sme. */
    size_t nth = bitmap_find_nth(smmu.sme_bitmap, smmu.sme_num, 1, 0, false);
    if (nth >= 0) {
