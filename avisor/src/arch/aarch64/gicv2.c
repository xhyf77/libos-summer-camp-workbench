...
static inline void gicc_init() {
    ...
    ssize_t gic_cpu_id = bit32_ffs(sgi_targets) + 1;

    if (gic_cpu_id < 0) {
        ERROR("Can't find gic cpu id");
    }
...
void gic_map_mmio() {
    // gicc = (void*) mem_alloc_map_dev(&cpu()->as, INVALID_VA,
    //     platform.arch.gic.gicc_addr, NUM_PAGES(sizeof(struct gicc_hw)));
    // gich = (void*) mem_alloc_map_dev(&cpu()->as, INVALID_VA,
    //     platform.arch.gic.gich_addr, NUM_PAGES(sizeof(struct gich_hw)));
    // gicd = (void*) mem_alloc_map_dev(&cpu()->as, INVALID_VA,
    //     platform.arch.gic.gicd_addr,  NUM_PAGES(sizeof(struct gicd_hw)));
    gicc = (void *)platform.arch.gic.gicc_addr;
    gich = (void *)platform.arch.gic.gich_addr;
    gicd = (void *)platform.arch.gic.gicd_addr;
}
...
void gic_set_pend(irqid_t int_id, bool pend) {
    ...
    if (gic_is_sgi(int_id)) {
        reg_ind = GICD_SGI_REG(int_id);
        off = GICD_SGI_OFF(int_id);
