...
bool vgicd_set_trgt(struct vcpu *vcpu, struct vgic_int *interrupt, cpumap_t targets) {
    uint8_t prev_targets;

    if (gic_is_priv(interrupt->id)) {
        return false;
    }

    prev_targets = interrupt->targets;
...
void vgic_inject_sgi(struct vcpu *vcpu, struct vgic_int *interrupt, vcpuid_t source) {
    ...
    pendstate = interrupt->sgi.pend;
    new_pendstate = pendstate | (1U << source);
...
void vgic_init(struct vm *vm, const struct vgic_dscrp *vgic_dscrp) {
    ...
    vtyper_itln = vgic_get_itln(vgic_dscrp);
    ...
    n = NUM_PAGES(sizeof(struct gicc_hw));
    // 提供虚假的地址
    mem_alloc_map_dev(&vm->as, (vaddr_t)vgic_dscrp->gicc_addr, 
