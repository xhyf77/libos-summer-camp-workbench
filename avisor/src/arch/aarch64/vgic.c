...
static inline int64_t gich_get_lr(struct vgic_int *interrupt, unsigned long *lr) {
    ...
    if (!interrupt->in_lr || interrupt->owner->p_id != cpu()->id) {
        return -1;
    }

    lr_val = gich_read_lr(interrupt->lr);
...
void vgic_route(struct vcpu *vcpu, struct vgic_int *interrupt) {
    ...
    if (!interrupt->in_lr && vgic_int_has_other_target(vcpu, interrupt)) {
        ...
        trgtlist = vgic_int_ptarget_mask(vcpu, interrupt) & ~(1ull << vcpu->p_id);
        for (size_t i = 0; i < platform.cpu_num; i++) {
            if (trgtlist & (1ull << i)) {
                cpu_send_msg(i, &msg);
            }
...
static inline void vgic_write_lr(struct vcpu *vcpu, struct vgic_int *interrupt,
    ...
    struct vgic_int *prev_interrupt = NULL;
    unsigned state;

    if ((prev_int_id != interrupt->id) && !gic_is_priv(prev_int_id)) {
        prev_interrupt = vgic_get_int(vcpu, prev_int_id, vcpu->id);
...
void vgic_add_spilled(struct vcpu *vcpu, struct vgic_int* interrupt) {
    struct list_head *spilled_list = NULL;
    ...
    list_add_tail(&interrupt->list, spilled_list);
...
bool vgic_add_lr(struct vcpu *vcpu, struct vgic_int *interrupt) {
    ...
    ssize_t lr_ind;
    uint64_t elrsr;
    unsigned min_prio_pend, min_prio_act;
    unsigned min_id_act, min_id_pend;
    size_t pend_found, act_found;
    ssize_t pend_ind, act_ind;
    ...
    if (lr_ind < 0) {
        min_prio_pend = interrupt->prio, min_prio_act = interrupt->prio;
        min_id_act = interrupt->id, min_id_pend = interrupt->id;
        pend_found = 0, act_found = 0;
        pend_ind = -1, act_ind = -1;

        for (size_t i = 0; i < NUM_LRS; i++) {
            lr = gich_read_lr(i);
            lr_id = GICH_LR_VID(lr);
            lr_prio = (lr & GICH_LR_PRIO_MSK) >> GICH_LR_PRIO_OFF;
            if (GIC_VERSION == GICV2) {
                lr_prio = lr_prio << 3;
            }
            lr_state = (lr & GICH_LR_STATE_MSK);
    ...
    if (vgic_get_ownership(vcpu, interrupt)) {
        ...
        struct cpu_msg msg = {VGIC_IPI_ID, VGIC_SET_REG,
                         VGIC_MSG_DATA(vcpu->vm->id, 0, interrupt->id,
                                       handlers->regid, data)};
        cpu_send_msg(interrupt->owner->p_id, &msg);
    }
...
void vgic_emul_generic_access(struct emul_access *acc,
    ...
    if (valid_access) {
        for (size_t i = 0; i < ((acc->width * 8) / field_width); i++) {
            interrupt = vgic_get_int(cpu()->vcpu, first_int + i, vgicr_id);
            if (interrupt == NULL) {
                break;
            }
...
struct vgic_int* vgic_highest_prio_spilled(struct vcpu *vcpu, 
    ...
    struct vgic_int* temp_irq = NULL;
    struct list_head* spilled_lists[] = {
        &vcpu->arch.vgic_spilled,
        &vcpu->vm->arch.vgic_spilled,
    };
    size_t spilled_list_size = sizeof(spilled_lists) / sizeof(struct list_head*);

    for(size_t i = 0; i< spilled_list_size; i++) {
        struct list_head *list = spilled_lists[i];
        list_for_each_entry(temp_irq, list, list) {
