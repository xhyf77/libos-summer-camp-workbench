...
SYSREG_GEN_ACCESSORS(CurrentEL);
...
static inline void arm_dc_civac(size_t cache_addr) {
    asm volatile ("dc civac, %0\n\t" :: "r"(cache_addr));
}

static inline void arm_at_s1e2w(size_t vaddr) {
     asm volatile("at s1e2w, %0" ::"r"(vaddr));
}

static inline void arm_at_s12e1w(size_t vaddr) {
...
static inline void arm_tlbi_vae2is(size_t vaddr) {
    asm volatile("tlbi vae2is, %0" ::"r"(vaddr >> 12));
}

static inline void arm_tlbi_ipas2e1is(size_t vaddr) {
